...
//   const fixedItems = sliceItems.map((item) => ({ ...item, id: String(item.id) }));
...
const TimeLineItemsStyled = () => {
  ...
  const fixedItemsIds = sliceItems.map((item) => ({ ...item, id: String(item.id) })); // для react-timeline-calendar нужно перевести id в строку (иначе id 0 будет восприниматься как false)
